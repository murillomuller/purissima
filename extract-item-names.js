...
function extractItemNames(orders) {
    ...
    for (const [orderId, orderData] of Object.entries(orders)) {
        ...
        } else if (Array.isArray(orderData) && orderData.length > 0 && typeof orderData[0] === 'object') {
...
function generateReports(extractionResults) {
    const { itemNames, itemDetails, totalItems, ordersProcessed } = extractionResults;
    ...
    fs.writeFileSync(detailsFile, JSON.stringify(detailsContent, null, 2), 'utf8');
    files.details = detailsFile;

    // 4. Raw orders data (JSON)
    const rawFile = `raw-orders-data_${timestamp}.json`;
    fs.writeFileSync(rawFile, JSON.stringify(extractionResults.rawOrders, null, 2), 'utf8');
    files.raw = rawFile;
...
function displayResults(results, extractionResults) {
    ...
    if (uniqueItemNames.length === 0) {
        log('\nNo item names found. The API response structure might be different than expected.', 'yellow');
        return;
    }
...
async function main() {
    try {
        ...
        if (!data.status || data.status !== 1) {
            throw new Error(`API returned error status: ${data.message || 'Unknown error'}`);
        }
    ...
    log('\n=== SUMMARY ===', 'cyan');
    log(`✓ Successfully extracted ${results.uniqueItemNames.length} unique item names`, 'green');
    log(`✓ Processed ${extractionResults.ordersProcessed} orders with ${extractionResults.totalItems} total items`, 'green');
