...
<div class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
    ...
    <div class="bg-gradient-to-r from-primary to-secondary shadow-lg">
        <div class="px-2 sm:px-4 lg:px-8 py-6">
            ...
            <div class="flex flex-col space-y-4">
                ...
                <div class="flex-1">
                    <div class="relative">
                        <input id="searchInput" type="text" placeholder="Buscar itens..." class="w-full px-4 py-3 rounded-lg text-sm placeholder-gray-300 text-gray-800 focus:outline-none focus:ring-2 focus:ring-white/60 focus:bg-white/90 bg-white/80 transition-opacity duration-200" />
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    ...
                    <div class="flex flex-col sm:w-48">
                        ...
                        <input id="dateFromFilter" type="datetime-local" onchange="applyFilters()" class="bg-white/90 text-gray-800 text-sm rounded-lg px-3 py-2 border border-white/50 focus:outline-none focus:ring-2 focus:ring-white/80 shadow-sm" />
                    </div>

                    <!-- Date To Filter -->
                    <div class="flex flex-col sm:w-48">
                        <label class="text-white text-sm font-semibold mb-2">Data final</label>
                        <input id="dateToFilter" type="datetime-local" onchange="applyFilters()" class="bg-white/90 text-gray-800 text-sm rounded-lg px-3 py-2 border border-white/50 focus:outline-none focus:ring-2 focus:ring-white/80 shadow-sm" />
                    </div>

                    <!-- Limit Filter -->
                    <div class="flex flex-col sm:w-48">
                        <label class="text-white text-sm font-semibold mb-2">Limite</label>
                        <input id="limitFilter" type="number" placeholder="Sem limite" onchange="applyFilters()" class="bg-white/90 text-gray-800 text-sm rounded-lg px-3 py-2 border border-white/50 focus:outline-none focus:ring-2 focus:ring-white/80 shadow-sm" />
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="px-2 sm:px-4 lg:px-8 py-4 sm:py-6 max-w-full">
        ...
        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-end mb-4 sm:mb-6 gap-3 sm:gap-3">
            <!-- Refresh Button -->
            <button onclick="refreshProduction()"
                ...
                <span class="text-sm sm:text-base">Atualizar Produção</span>
            </button>
        ...
        <div class="bg-white rounded-lg shadow-sm border mb-6">
            <div class="border-b border-gray-200">
                <nav class="-mb-px flex space-x-8 px-6">
                    <button onclick="switchTab('ativos')" class="tab-button active py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200" data-tab="ativos">
                    ...
                    <button onclick="switchTab('removidos')" class="tab-button py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200" data-tab="removidos">
                        <div class="flex items-center space-x-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button onclick="switchTab('itens')" class="tab-button py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200" data-tab="itens">
                        <div class="flex items-center space-x-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button onclick="switchTab('producao')" class="tab-button py-4 px-1 border-b-2 font-medium text-sm transition-colors duration-200" data-tab="producao">
                        <div class="flex items-center space-x-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.806.547M8 4h8l-1 1v5.172a2 2 0 00.586 1.414l5 5c1.26 1.26.367 3.414-1.415 3.414H4.828c-1.782 0-2.674-2.154-1.414-3.414l5-5A2 2 0 009 10.172V5L8 4z"></path>
                            </svg>
                    ...
                    <div class="p-6">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <div class="flex gap-3">
                                ...
                                <button onclick="downloadItemsReport()" class="bg-green-50 hover:bg-green-100 text-green-700 px-4 py-2 rounded-lg text-sm font-medium transition-colors duration-200 flex items-center space-x-2">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<script>
    ...
    function updateCounts(data) {
        ...
        document.getElementById('removed-count').textContent = 0; // Will be updated when we implement removed orders
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            <div class="bg-blue-50 rounded-lg p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <div class="text-2xl font-bold text-blue-900">${totals.required.toLocaleString()}</div>
            ...
            <div class="bg-green-50 rounded-lg p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <div class="text-2xl font-bold text-green-900">${totals.produced.toLocaleString()}</div>
            ...
            <div class="bg-orange-50 rounded-lg p-4">
                <div class="flex items-center">
                    ...
                    <div class="ml-4">
                        <div class="text-2xl font-bold text-orange-900">${totals.remaining.toLocaleString()}</div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Produzido</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Restante</th>
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                    <div class="flex items-center">
                        <div class="w-16 bg-gray-200 rounded-full h-2 mr-2">
                            <div class="bg-green-600 h-2 rounded-full" style="width: ${Math.min(100, itemProgress)}%"></div>
                        </div>
                        <span class="text-xs text-gray-500">${itemProgress.toFixed(1)}%</span>
                    </div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <button onclick="updateProductionQuantity('${item.item}', ${item.totalQuantity}, ${item.producedQuantity})" 
    ...
    function selectAllRemovedOrders() {
        // Implementation for removed orders selection
    }

    function restoreSelectedOrders() {
        // Implementation for restoring orders
    }
