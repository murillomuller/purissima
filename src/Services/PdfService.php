...
class PdfService
{
    ...
    public function createPrescriptionPdf(array $orderData, array $items): string
    {
        ...
        try {
            ...
            $pdf->SetXY(20, 50);
            $orderDate = $orderData['created_at'] ?? date('d/m/Y H:i');
            if ($orderDate !== date('d/m/Y H:i')) {
                // Convert from Y-m-d H:i:s format to d/m/Y H:i format
                $orderDate = date('d/m/Y H:i', strtotime($orderDate));
            }
            $pdf->Cell(0, 8, 'Data: ' . $orderDate, 0, 1);
